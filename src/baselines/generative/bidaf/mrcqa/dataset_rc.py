...
class EpochGen(object):
    ...
    def __iter__(self):
        ...
        for start_ind in range(0, self.n_samples - 1, self.batch_size):
            ...
            # Adding answer descriptions here
            outputs = [self.data[ind][3][0] for ind in batch_idx]
            c_outputs = [self.data[ind][3][1] for ind in batch_idx]
            
            answers = [self.data[ind][4] for ind in batch_idx]
            ...
            queries = self.process_batch_for_length(
                    queries, c_queries)
            outputs = self.process_batch_for_length(
                    outputs, c_outputs)
